<template>
  <section :style="styling" v-if="isValid" class="photo-capture">
    ...
    <div v-if="!hideBtns" class="center photo-capture-actions">
      <v-btn color="primary" @click.prevent="capture" v-if="showVideo">Capturar selfie</v-btn>
      ...
      <div class="controls" v-else>
        <v-btn color="warning" @click.prevent="cancel">Nova selfie</v-btn>
...
export default {
  ...
  methods: {
    ...mapMutations({
        setCrediario: "crediario/setCrediario"
    }),
    ...
    capture() {
      ...
      this.setCrediario({...this.getCrediario, foto_selfie: this.picture})
    },
    done() {
    ...
    cancel() {
      this.showVideo = true;
      this.streamUserMediaVideo();
      this.setCrediario({...this.getCrediario, foto_selfie: null})
    },
    stopVideoStream() {
